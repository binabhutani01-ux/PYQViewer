...
| `MAPTILER_API_KEY` | optional MapTiler server-side key | used only to reverse-geocode GPS coordinates; if omitted, the server falls back to OpenStreetMap Nominatim |
| `OWNER1_USER` | `Dev1` | |
...
- **A dark, minimal map** in the owner console (Login Map tab) plots every login by lat/long on a CARTO dark basemap, with a popup showing IP, city, device, time, location source, and GPS accuracy when available.
...
## 9. September 2026 fixes
- GPS coordinates are now authoritative for city/region/country whenever a teacher grants browser location permission. The server reverse-geocodes the coordinates; it no longer trusts a city name supplied by the browser or silently replaces a valid GPS city with the IP city.
- The main page asks for GPS without doing a second client-side reverse-geocode call, reducing sign-in latency.
- Teacher colours in the owner Login Map are assigned deterministically and uniquely across the current teacher directory using hue spacing. Colours are owner-console/map-only and are not shown to teachers.
- Page/list animations were de-duplicated and the heavy animated dot canvas remains, but navigation no longer re-animates every worksheet row.
- The supplied PYQViewer logo is available at `/assets/pyqviewer-icon.svg` and is used as the site favicon, main brand icon, owner-console icon, and the custom PDF viewer icon/header.
- Sub-subjects can now be deleted by their creator or the head teacher. Deleting one also removes its worksheets and storage files.
- The owner console can create approved subjects and sub-subjects under any approved teacher's name, and subject deletion now removes related worksheets/files too.
- The worksheet picker was rebuilt around a native iOS-compatible `<label>` + file input, accepts `.pdf` even when iOS reports an empty MIME type, displays the selected filename immediately, and retries a failed storage upload once.

Run no new SQL migration for these fixes; they use the existing `created_by`, `sub_subject_id`, `file_path`, and location columns already introduced by the previous migrations.
