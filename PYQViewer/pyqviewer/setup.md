...
- **Real IP-based geolocation**, not the old always-"Unknown" lookup. It now reads Vercel's own edge geo headers first (instant, free, no rate limits), with the old third-party API as a fallback for local dev. This gives city + region + country + lat/long — not device GPS, since silently pulling a device's actual GPS without the person's consent isn't something this build does.
- **A dark, minimal map** in the owner console (Login Map tab) plots every login by lat/long on a CARTO dark basemap, with a popup showing exact IP, city, device, and time per pin.
