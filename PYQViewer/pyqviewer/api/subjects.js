...
module.exports = async (req, res) => {
  try {
    ...
    if (body.action === 'delete_subsubject') {
      ...
      const { data: ss } = await supabase.from('sub_subjects').select('id,subject_id').eq('id', id).maybeSingle();
      if (!ss) return res.status(200).json({ success: false, error: 'Sub-subject not found.' });
      // Preserve worksheet records; only clear their now-invalid label.
      const { error: clearErr } = await supabase.from('worksheets').update({ sub_subject_id: null }).eq('sub_subject_id', id);
      if (clearErr) return res.status(200).json({ success: false, error: clearErr.message });
