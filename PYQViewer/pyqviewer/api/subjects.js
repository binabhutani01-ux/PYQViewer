...
module.exports = async (req, res) => {
  try {
    ...
    if (body.action === 'delete_subsubject') {
      ...
      // Only head teachers or the creator can delete a sub-subject
      const { data: ss } = await supabase.from('sub_subjects').select('created_by').eq('id', id).maybeSingle();
      if (!ss) return res.status(200).json({ success: false, error: 'Sub-subject not found.' });
      if (session.role !== 'head' && ss.created_by !== session.id) {
        return res.status(200).json({ success: false, error: 'Only the creator or a head teacher can delete a sub-subject.' });
      }
      // Nullify sub_subject_id on any worksheets using this sub-subject (don't delete the files)
