...
const {
  ...
  getClientIp, lookupGeo, reverseGps, parseDevice,
} = require('../lib/auth');
...
module.exports = async (req, res) => {
  try {
    ...
    const pictureUrl = typeof payload.picture === 'string' && /^https:\/\//i.test(payload.picture) ? payload.picture : null;
    ...
    // Keep the latest Google profile photo on the teacher record.
    // This is cosmetic only; if the optional column is not migrated yet,
    ...
    if (pictureUrl) {
      try {
        ...
        console.warn('Could not save Google profile picture:', e.message);
      }
    }
    ...
    const gpsPlace = hasValidGps ? await reverseGps(gpsLat, gpsLon) : null;
    const device = parseDevice(req.headers['user-agent']);
    const deviceKey = crypto.createHash('sha256').update(`${teacher.id}|${device}|${hasValidGps ? gpsLat.toFixed(4)+','+gpsLon.toFixed(4) : geo.city}|${geo.country}`).digest('hex');
    ...
    const eventRow = {
      ...
      ip,
      city: hasValidGps && gpsPlace?.city ? String(gpsPlace.city) : geo.city,
      region: hasValidGps && gpsPlace?.region ? String(gpsPlace.region) : geo.region,
      country: hasValidGps && gpsPlace?.country ? String(gpsPlace.country) : geo.country,
