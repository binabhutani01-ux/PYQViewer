...
module.exports = async (req, res) => {
  try {
    ...
    if (body.action === 'create_subject_as') {
      ...
      const { data, error } = await supabase.from('subjects').insert([{
        ...
        name,
        created_by: teacher ? teacher.id : 'owner',
        created_by_name: teacher ? teacher.name : 'Owner',
        status: 'approved',
    ...
    if (body.action === 'create_subsubject_as') {
      const name = (body.name || '').trim();
      const subjectId = body.subject_id;
      const teacherId = body.teacher_id;
      if (!name || !subjectId) return res.status(200).json({ success: false, error: 'Name and parent subject required.' });
      const { data: teacher } = await supabase.from('teachers').select('id,name').eq('id', teacherId).maybeSingle();
      const crypto = require('crypto');
    ...
    if (body.action === 'delete_subsubject') {
      const id = body.id;
      if (!id) return res.status(200).json({ success: false, error: 'Sub-subject id required.' });
    ...
    if (body.action === 'get_site_settings') {
      const { data } = await supabase.from('settings').select('key,value').in('key', ['site_theme', 'site_font', 'maintenance_mode']);
      const settings = {};
      (data || []).forEach(row => { settings[row.key] = row.value; });
      return res.status(200).json({ success: true, settings });
    }

    if (body.action === 'set_site_setting') {
      const key = body.key;
      const value = String(body.value || '');
      const ALLOWED_KEYS = ['site_theme', 'site_font'];
      if (!ALLOWED_KEYS.includes(key)) return res.status(200).json({ success: false, error: 'Invalid setting key.' });
