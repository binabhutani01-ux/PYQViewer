...
const { getSession, parseBody } = require('../lib/auth');
...
module.exports = async (req, res) => {
  try {
    ...
    // Everything below requires an approved, signed-in teacher
    if (!session) return res.status(200).json({ success: false, error: 'You must be signed in as a teacher.' });
