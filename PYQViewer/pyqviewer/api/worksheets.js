...
module.exports = async (req, res) => {
  try {
    ...
    // ---- SINGLE WORKSHEET (public) ----
    if (action === 'get_one') {
      const id = body.id;
      if (!id) return res.status(200).json({ success: false, error: 'Missing worksheet id.' });
      const { data, error } = await supabase.from('worksheets').select('*').eq('id', id).maybeSingle();
      if (error) return res.status(200).json({ success: false, error: error.message });
      if (!data) return res.status(200).json({ success: false, error: 'Worksheet not found.' });
      return res.status(200).json({ success: true, worksheet: data });
    }
