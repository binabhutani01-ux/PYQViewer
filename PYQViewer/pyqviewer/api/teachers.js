...
module.exports = async (req, res) => {
  try {
    ...
    if (body.action === 'login_events') {
      // Precise IP/location is dev-console-only now (see api/owner.js → 'overview').
      // Even the head teacher doesn't get this from here anymore.
      return res.status(200).json({ success: false, error: 'Login location details are only available in the developer console.' });
    }
